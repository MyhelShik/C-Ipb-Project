...
void AdicionarLivro()
{
    ...
    for (int i = 0; i < max_livros; i++) {
        ...
        novolivro = (livro *)malloc(sizeof(livro));  // Aloca memÃ³ria para um livro
        if (novolivro == NULL) {
        ...
        scanf(" %c", &continuar);  // LÃª a resposta
...
void PesquisarLivro() {
    ...
    int encontrado = 0;
    for (int i = 0; i < num_livros_adicionados; i++) {
        if (strstr(livros[i]->titulo, titulo_busca) != NULL) {  // Verifica se o tÃ­tulo contÃ©m o texto de busca
